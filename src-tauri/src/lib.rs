...
use crate::{
    db::{
        approve_proposal, generate_study_proposal, init_database, list_history, list_proposals,
        list_browser_aliases, list_learned_intents, list_proposal_steps, list_routines, list_voice_corrections,
        get_executor_config, get_google_calendar_config, get_notion_config, get_spotify_config, log_action, reject_proposal, save_browser_alias,
        save_executor_config, save_learned_intent, save_local_tts_backend_config, save_ollama_config,
        save_google_calendar_config, save_local_voice_backend_config, save_notion_config, save_spotify_config, save_voice_correction, update_proposal,
        get_local_tts_backend_config, get_local_voice_backend_config, get_ollama_config,
        get_wake_mode_config, save_wake_mode_config,
    },
    models::{
        BrowserAliasRecord, BuildHandoffArtifact, ConversationInterpretation,
        ConversationInterpretationRequest, ExecutorLaunchRequest, ExecutorStatus, HistoryRecord,
        FileRecord, GoogleCalendarStatus, LearnedIntentRecord, LocalSpeechOutputStatus, LocalVoiceBackendStatus, MissingSkillPlan, NoteRecord,
        MissingSkillPlanRequest, OllamaStatus, ProposalRecord, ProposalStepRecord,
        ProposalUpdateInput, RoutineRecord, SkillBuildRequestInput, SpotifyStatus, VoiceCorrectionRecord,
        WakeModeStatus, NotionStatus,
    },
};
...
fn map_notion_note_record(
    ...
    let last_edited_time = page
        ...
        .unwrap_or_default()
        .to_string();
    let status_property = find_notion_property_name(database, &["Status"], &["status", "select", "rich_text"]);
    let due_property =
        find_notion_property_name(database, &["Due", "Due Date", "Deadline"], &["date", "rich_text"]);

    let mut summary_lines = Vec::new();
    if let Some((property_name, property_type)) = status_property {
        if let Some(value) = extract_notion_property_display_value(page, &property_name, &property_type) {
...
fn request_ollama_response_text(
    ...
    extract_ollama_response_text(&fallback_response)
        .ok_or_else(|| format!("Ollama returned an empty response. Raw payload: {}", fallback_response))
}

fn parse_ollama_json_response<T: DeserializeOwned>(content: &str, context: &str) -> Result<T, String> {
...
fn normalize_conversation_interpretation(content: &str) -> Result<ConversationInterpretation, String> {
    if let Ok(parsed) = parse_ollama_json_response::<ConversationInterpretation>(content, "interpretation") {
        return Ok(parsed);
    }
    ...
    let kind = get_string_alias(&value, &["kind", "intent", "type", "action"])
        .ok_or_else(|| format!("Ollama interpretation JSON is missing a kind field: {}", json_text))?;
    ...
    let clarification_prompt = get_string_alias(
        &value,
        &["clarificationPrompt", "clarification_prompt", "question", "followUp", "follow_up"],
    );
...
fn normalize_missing_skill_plan(content: &str) -> Result<MissingSkillPlan, String> {
    if let Ok(parsed) = parse_ollama_json_response::<MissingSkillPlan>(content, "missing-skill plan") {
        return Ok(parsed);
    }
    ...
    let skill_name = get_string_alias(
        &value,
        &["skillName", "skill_name", "name", "title"],
    )
    .ok_or_else(|| format!("Ollama missing-skill JSON is missing skillName: {}", json_text))?;
    ...
    let user_value = get_string_alias(&value, &["userValue", "user_value", "value", "benefit"])
        .unwrap_or_else(|| "This would let JARVIS handle the request directly next time.".to_string());

    let build_steps = {
        let steps = get_string_array_alias(
            &value,
            &["buildSteps", "build_steps", "steps", "implementationSteps", "implementation_steps"],
        );
        if steps.is_empty() {
            vec![
    ...
    let permissions_needed = get_string_array_alias(
        &value,
        &["permissionsNeeded", "permissions_needed", "permissions", "approvals"],
    );
    let approval_message = get_string_alias(
        &value,
        &["approvalMessage", "approval_message", "confirmation", "confirmMessage", "confirm_message"],
    )
    .unwrap_or_else(|| format!("Do you want me to prepare the {} skill for implementation?", skill_name));
...
#[tauri::command]
fn get_voice_corrections(
    state: State<'_, AppState>,
) -> Result<Vec<VoiceCorrectionRecord>, String> {
...
#[tauri::command]
fn get_local_voice_backend_status(state: State<'_, AppState>) -> Result<LocalVoiceBackendStatus, String> {
...
fn get_notion_status(state: State<'_, AppState>) -> Result<NotionStatus, String> {
    ...
    let configured = has_token && database_id.as_ref().is_some_and(|value| !value.trim().is_empty());
...
fn get_google_calendar_status(state: State<'_, AppState>) -> Result<GoogleCalendarStatus, String> {
    ...
    let has_client_id = client_id.as_ref().is_some_and(|value| !value.trim().is_empty());
    ...
    Ok(GoogleCalendarStatus {
        ...
        message: if configured {
            ...
            "Set a Google Calendar client ID and API key to enable direct calendar API access.".to_string()
        },
    })
}
...
fn get_spotify_status(state: State<'_, AppState>) -> Result<SpotifyStatus, String> {
    ...
    Ok(SpotifyStatus {
        ...
        message: if has_client_id {
            "Spotify client ID is saved. Connect in the app to enable playback controls.".to_string()
        } else {
...
fn create_notion_note(
    ...
    let title_text = trimmed_content.chars().take(60).collect::<String>();
...
fn update_notion_note_title(
    ...
    if !update_response.status().is_success() {
        ...
        return Err(format!("Notion note update failed with {}: {}", status, body));
    }
    ...
    Ok(map_notion_note_record(&page_value, &title_property_name, &database_value))
}
...
fn create_notion_task(
    ...
    let database_value: serde_json::Value = database_response
        ...
        .map_err(|error| format!("Failed to parse Notion database metadata: {}", error))?;
    let title_property_name = extract_notion_title_property(&database_value)?;
    let has_status_metadata =
        find_notion_property_name(&database_value, &["Status"], &["status", "select", "rich_text"]).is_some();
    let has_due_metadata =
        find_notion_property_name(&database_value, &["Due", "Due Date", "Deadline"], &["date", "rich_text"]).is_some();
    ...
    if !create_response.status().is_success() {
        let status = create_response.status();
        let body = create_response.text().unwrap_or_default();
        return Err(format!("Notion task creation failed with {}: {}", status, body));
    }
...
fn update_notion_task(
    ...
    if !update_response.status().is_success() {
        ...
        let body = update_response.text().unwrap_or_default();
        return Err(format!("Notion task update failed with {}: {}", status_code, body));
    }
...
fn get_ollama_status(state: State<'_, AppState>) -> Result<OllamaStatus, String> {
    ...
    let configured = base_url.as_ref().is_some_and(|value| !value.trim().is_empty())
        && model_name.as_ref().is_some_and(|value| !value.trim().is_empty());
    ...
    if !configured {
        return Ok(OllamaStatus {
            ...
            base_url,
            model_name,
            message: "Set an Ollama base URL and model name to enable local conversation interpretation."
                .to_string(),
        });
    }
...
fn create_build_handoff_artifact(
    ...
    Ok(BuildHandoffArtifact {
        ...
        created_at: request.created_at,
        message:
            "JARVIS created a coding handoff package. Manual execution is the next boundary."
                .to_string(),
    })
}
...
fn get_executor_status(state: State<'_, AppState>) -> Result<ExecutorStatus, String> {
    ...
    let configured = command_path.as_ref().is_some_and(|value| !value.trim().is_empty());
...
#[cfg(target_os = "windows")]
fn open_windows_target(target: StudyTarget) -> Result<(), String> {
    Command::new("cmd")
