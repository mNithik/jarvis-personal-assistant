...
pub fn upsert_domain_entity(
    ...
    let entity_id = if let Ok(existing_id) = connection.query_row(
        "SELECT id FROM memory_entities WHERE domain = ?1 AND lower(label) = lower(?2)",
        params![domain, label],
        |row| row.get::<_, i64>(0),
    ) {
        connection
        ...
        connection
            .execute(
                "INSERT INTO memory_entities (domain, label, metadata_json) VALUES (?1, ?2, ?3)",
                params![domain, label, metadata_json],
            )
            .map_err(|error| error.to_string())?;
    ...
    if !summary.trim().is_empty() {
        connection
            .execute(
                "INSERT INTO memory_documents (domain, source, content) VALUES (?1, 'import', ?2)",
                params![domain, summary],
            )
            .map_err(|error| error.to_string())?;
...
pub fn list_entity_metadata(path: &Path, domain: &str) -> Result<Vec<(i64, String)>, String> {
    ...
    let mut statement = connection
        .prepare(
            "SELECT id, metadata_json FROM memory_entities WHERE domain = ?1 ORDER BY updated_at DESC",
        )
        .map_err(|error| error.to_string())?;
